...
export const processTransaction:(req:Request, res:Response) => void = async (req,res) =>{
    ...
    if(transactionRequest.transactionEnv == TransactionEnv.ATM){
        ...
        const request:VendorTransactionRequest = new VendorTransactionRequest(transactionRequest)
        ...
        res.status(400).json("");
    }else if(transactionRequest.transactionEnv == TransactionEnv.MOBILE || transactionRequest.transactionEnv == TransactionEnv.ONLINE){
        if(!token || !transactionRequest.apiKey){
            res.status(401).json("Unauthorized");
            return;
        }
        const request:AccountTransferRequest = new AccountTransferRequest(transactionRequest);
